...
/**
 * The Model class represents the state of the Game of Life application.
 * It contains the current board state and the logic to update the board
...
public final class Model {
    ...
    public Model() {
        ...
        simulation = new Timer(INITIAL_DELAY_IN_MS, e -> nextGeneration());
    }

    public void newBlankGame() {
        currentGeneration = 0;
        currentBoard = new boolean[ROWS][COLS];
        nextBoard = new boolean[ROWS][COLS];
        notifyObservers();
    ...
    public void setSimulationSpeed(int delayInMs) {
        assert delayInMs >= MIN_DELAY_IN_MS && delayInMs <= MAX_DELAY_IN_MS;
     ...
     *
     * @param view the observer to be removed
     */
